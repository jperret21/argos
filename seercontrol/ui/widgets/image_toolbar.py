...
Provides quick access to channel selection, exposure/gain settings,
gamma correction, and auto-stretch — displayed in a single 36px-high bar.
"""
...
def _sep() -> QFrame:
    """Return a compact vertical separator line."""
    line = QFrame()
...
class ImageToolbar(QWidget):
    ...
    Signals:
        ...
        exposure_changed:      Emitted when the exposure spinbox changes.
        gain_changed:          Emitted when the gain spinbox changes.
    ...
    def _build_ui(self) -> None:
        ...
        # ── Exposure ───────────────────────────────────────────────────
        layout.addWidget(_lbl("Exp:"))
        self._exposure_spin = QDoubleSpinBox()
        self._exposure_spin.setFixedHeight(24)
        self._exposure_spin.setFixedWidth(80)
        self._exposure_spin.setStyleSheet("font-size: 11px;")
        ...
        self._exposure_spin.valueChanged.connect(self.exposure_changed)
        layout.addWidget(self._exposure_spin)

        # ── Gain ───────────────────────────────────────────────────────
        layout.addWidget(_lbl("Gain:"))
        self._gain_spin = QSpinBox()
        self._gain_spin.setFixedHeight(24)
        self._gain_spin.setFixedWidth(65)
        self._gain_spin.setStyleSheet("font-size: 11px;")
        self._gain_spin.setRange(0, 600)
        self._gain_spin.setValue(80)
        self._gain_spin.valueChanged.connect(self.gain_changed)
        ...
        self._gamma_slider.setFixedWidth(90)
        self._gamma_slider.setRange(10, 300)
        self._gamma_slider.setValue(100)
        self._gamma_slider.setToolTip("Gamma correction: 0.1 (dark boost) – 3.0 (bright boost)")
    ...
    def set_gain_range(self, min_val: int, max_val: int) -> None:
        """Update the gain spinbox range (call after camera connects)."""
        self._gain_spin.setRange(min_val, max_val)

    def set_gain(self, value: int) -> None:
        """Set gain spinbox value without emitting signal."""
        self._gain_spin.blockSignals(True)
        self._gain_spin.setValue(value)
        self._gain_spin.blockSignals(False)

    def set_exposure(self, value: float) -> None:
        """Set exposure spinbox value without emitting signal."""
