"""Stellarium integration card — TCP telescope-control server toggle.

Embedded in the Connection panel as a Siril-style card (QGroupBox). SeerControl
runs the standard Stellarium *Telescope Control* TCP server: Stellarium connects
to it as a telescope, and a slew command (select an object, press Ctrl+1 / ⌘+1)
sends a goto that SeerControl forwards to the mount. The card does not own the
worker; it emits intents (start/stop) and accepts state updates.
"""
...
class StellariumCard(QGroupBox):
    """Compact control panel for the Stellarium telescope-control server.
    ...
    def _build_ui(self, tcp_port: int) -> None:
        ...
        hint.setStyleSheet(
            f"color:{theme.FG_MUTED}; font-size:10px;" f" background:transparent; padding-top:2px;"
        )
        outer.addWidget(hint)

    # ------------------------------------------------------------------
    # Slot API (called by Shell / worker signals)
    # ------------------------------------------------------------------
    ...
    def set_client_count(self, n: int) -> None:
        ...
        self._client_lbl.setStyleSheet(f"color:{color}; font-size:11px; background:transparent;")

    def flash_goto(self, ra_hours: float, dec_degrees: float) -> None:
        """Briefly highlight that a goto was received."""
        self._status_lbl.setText(f"↗  goto  RA {ra_hours:.4f}h  Dec {dec_degrees:+.4f}°")
